...
class resnet(object):
    ...
    def __init__(self, image_height, image_width, image_channel, keep_prob, classNum):
        self.X = tf.placeholder(tf.float32, [None, image_height, image_width],name='inputX')
    ...
    def buildCNN3(self):
        ...
        '''
        x = tf.unstack(self.X,11, 1)
        lstm_fw_cell = rnn.BasicLSTMCell(32, forget_bias=1.0)
        # Backward direction cell
        lstm_bw_cell = rnn.BasicLSTMCell(32, forget_bias=1.0)

        outputs, _, _ = rnn.static_bidirectional_rnn(lstm_fw_cell, lstm_bw_cell, x,
                                              dtype=tf.float32)
        flatten = tf.concat(outputs,axis=1)
        print('flatten:',flatten.shape)
        with tf.variable_scope('hidden4',regularizer=self.regularizer,initializer=self.init):
            dense = tf.layers.dense(flatten,units = 128)
